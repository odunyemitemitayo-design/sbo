...
const App: React.FC = () => {
  ...
  // Robust localStorage loader with safety checks
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem('safety_observations');
      if (saved) {
        setObservations(JSON.parse(saved));
      }
    } catch (e) {
      console.warn("Storage access denied by browser tracking prevention. Data will be session-only.", e);
    }
  }, []);
  ...
  const handleAddObservation = (newObs: Observation) => {
    ...
    try {
      window.localStorage.setItem('safety_observations', JSON.stringify(updated));
    } catch (e) {
